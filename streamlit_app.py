...
# FI and EQUITY asset groups shown in separate top-level tabs
...
FI_ASSETS     = ['TLT', 'LQD', 'HYG', 'VNQ', 'GLD', 'SLV']
EQUITY_ASSETS = ['QQQ', 'XLK', 'XLF', 'XLE', 'XLV', 'XLI',
...
# Color palette — FI gets blues/greens, Equity gets oranges/reds
COLORS_FI = {
...
COLORS_EQ = {
    'QQQ': '#FF6D00', 'XLK': '#FF9100', 'XLF': '#FFAB40',
    'XLE': '#8D6E63', 'XLV': '#66BB6A', 'XLI': '#42A5F5',
    'XLY': '#EF5350', 'XLP': '#AB47BC', 'XLU': '#26C6DA',
    'XME': '#78909C', 'GDX': '#FFC107', 'IWM': '#EC407A',
    'FTRL': '#E91E63', 'AGG': '#607D8B',
}
...
# ── Data loaders (group-aware) ────────────────────────────────────────────────

@st.cache_data(ttl=3600)
def load_summary(suffix: str) -> pd.DataFrame:
    frames = []
    for w_id in range(1, 15):
        try:
            path = hf_hub_download(repo_id=HF_REPO,
                filename=f"results/window_{w_id:02d}{suffix}_summary.json",
                repo_type="dataset")
            with open(path) as f:
...
@st.cache_data(ttl=3600)
def load_reverse_summary(suffix: str) -> pd.DataFrame:
    frames = []
    for w_id in range(1, 15):
        try:
            path = hf_hub_download(repo_id=HF_REPO,
                filename=f"results/reverse_window_{w_id:02d}{suffix}_summary.json",
                repo_type="dataset")
            with open(path) as f:
                frames.append(json.load(f))
        except Exception:
...
@st.cache_data(ttl=3600)
def load_window_daily(window_id: int, suffix: str) -> pd.DataFrame:
    try:
        path = hf_hub_download(repo_id=HF_REPO,
            filename=f"results/window_{window_id:02d}{suffix}_daily.csv",
            repo_type="dataset")
...
@st.cache_data(ttl=3600)
def load_all_daily(suffix: str) -> pd.DataFrame:
    frames = []
    for w_id in range(1, 15):
        df = load_window_daily(w_id, suffix)
        if not df.empty:
            df['window_id'] = w_id
            frames.append(df)
...
@st.cache_data(ttl=3600)
def load_reverse_window_daily(window_id: int, suffix: str) -> pd.DataFrame:
    try:
        path = hf_hub_download(repo_id=HF_REPO,
            filename=f"results/reverse_window_{window_id:02d}{suffix}_daily.csv",
            repo_type="dataset")
        return pd.read_csv(path, parse_dates=['date'], index_col='date')
    except Exception:
        return pd.DataFrame()
...
@st.cache_data(ttl=3600)
def load_all_reverse_daily(suffix: str) -> pd.DataFrame:
    frames = []
    for w_id in range(1, 15):
        df = load_reverse_window_daily(w_id, suffix)
        if not df.empty:
            df['window_id'] = w_id
            frames.append(df)
    return pd.concat(frames).sort_index() if frames else pd.DataFrame()
...
def load_latest_signal(suffix: str) -> dict:
    try:
        path = hf_hub_download(repo_id=HF_REPO,
            filename=f"results/latest_signal{suffix}.json",
...
@st.cache_data(ttl=300)
def load_latest_reverse_signal(suffix: str) -> dict:
    try:
        path = hf_hub_download(repo_id=HF_REPO,
            filename=f"results/latest_reverse_signal{suffix}.json",
            repo_type="dataset", force_download=True)
        with open(path) as f:
            return json.load(f)
    except Exception:
...
@st.cache_data(ttl=300)
def load_signal_history(suffix: str) -> pd.DataFrame:
    try:
        path = hf_hub_download(repo_id=HF_REPO,
            filename=f"results/signal_history{suffix}.json",
            repo_type="dataset", force_download=True)
        with open(path) as f:
            data = json.load(f)
        if not data:
...
@st.cache_data(ttl=300)
def load_reverse_signal_history(suffix: str) -> pd.DataFrame:
    try:
        path = hf_hub_download(repo_id=HF_REPO,
            filename=f"results/reverse_signal_history{suffix}.json",
            repo_type="dataset", force_download=True)
        with open(path) as f:
            data = json.load(f)
        if not data:
...
def signal_card(sig: dict, label: str, colors: dict, color_override=None) -> str:
    etf     = sig.get('signal', '—')
    conf    = sig.get('confidence', 0)
    dt      = sig.get('date', '—')
    trained = sig.get('trained_on', '—')
    c       = color_override or colors.get(etf, '#E91E63')
    return f"""
...
def compute_live_score(history_df: pd.DataFrame, n_total: int) -> tuple:
    ...
    if n == 0:
        return None, 0.0, 0.0, 0.0, 0, 0
    window_used = min(SCORE_WINDOW, n) if n_total >= MIN_DAYS_FULL else n
...
def consensus_label(n_scored: int) -> str:
    if n_scored < MIN_DAYS_PROVISIONAL:  return None
    if n_scored < MIN_DAYS_MODERATE:     return "provisional"
    if n_scored < MIN_DAYS_FULL:         return "moderate"
    return "full"


# ── Main renderer — one full group dashboard ──────────────────────────────────
...
def render_group(group_name: str, suffix: str, assets: list, colors: dict,
    ...
    reverse_summary_df = load_reverse_summary(suffix)
    all_daily          = load_all_daily(suffix)
    all_reverse_daily  = load_all_reverse_daily(suffix)
    signal_exp         = load_latest_signal(suffix)
    signal_rev         = load_latest_reverse_signal(suffix)
    history_exp        = load_signal_history(suffix)
    history_rev        = load_reverse_signal_history(suffix)

    exp_n_total  = len(history_exp) if not history_exp.empty else 0
    rev_n_total  = len(history_rev) if not history_rev.empty else 0
    exp_score, exp_avg_exc, exp_sharpe, exp_max_dd, exp_n, exp_win = \
        compute_live_score(history_exp, exp_n_total)
    rev_score, rev_avg_exc, rev_sharpe, rev_max_dd, rev_n, rev_win = \
        compute_live_score(history_rev, rev_n_total)
    min_n     = min(
        int(history_exp['beats_agg'].notna().sum()) if not history_exp.empty else 0,
        int(history_rev['beats_agg'].notna().sum()) if not history_rev.empty else 0,
    )
    ...
    if has_exp or has_rev:
        ...
        if has_exp and has_rev:
            ...
            with col_e:
                st.markdown(signal_card(signal_exp, "EXPANDING SIGNAL", colors),
                            unsafe_allow_html=True)
            with col_r:
                st.markdown(signal_card(signal_rev, "REVERSE SIGNAL", colors,
            ...
            with col_c:
                if con_level is None:
                    ...
                    if exp_score >= rev_score:
                        winner_etf, winner_label = exp_etf, "EXPANDING"
                        score_gap = exp_score - rev_score
                    else:
                        winner_etf, winner_label = rev_etf, "REVERSE"
            ...
            with col1:
                st.markdown(signal_card(signal_exp, "TODAY'S SIGNAL (EXPANDING)", colors),
                            unsafe_allow_html=True)
            with col2:
            ...
            with col1:
                st.markdown(
                    signal_card(signal_rev, "TODAY'S SIGNAL (REVERSE)", colors,
                                color_override='#FF9800'),
                    unsafe_allow_html=True)
            ...
            if not history_exp.empty and not history_rev.empty:
                ...
                if not merged.empty:
                    ...
                    merged['🤝']       = merged['Agree'].apply(
                        lambda x: "✅" if x else "❌")
                    c1, c2 = st.columns(2)
                    c1.metric("Agreement Rate", f"{merged['Agree'].mean():.0%}",
    ...
    if summary_df.empty:
        st.info(f"⏳ {group_name} back-test results not yet available. "
                "Run the training workflow first.")
        return

    # ── Overview banner ───────────────────────────────────────────────────────
    if 'excess_return' in summary_df.columns:
        has_live = ('live_excess_return' in summary_df.columns and
                    summary_df['live_excess_return'].notna().any())
        if has_live:
            pool     = summary_df[summary_df['live_excess_return'].notna()]
            best_exp = pool.loc[pool['live_excess_return'].idxmax()]
    ...
    if signal_exp:
        ...
        detail      = (f"from {train_start} → {train_end}" +
                       (f" · selected by {basis}" if basis else "")) \
                       if train_start and train_end else ""
        st.info(f"ℹ️ Today's expanding signal trained on: **{trained_on}**"
                + (f" ({detail})" if detail else ""))

    # ── Sub-tabs: Overview / Equity Curves / Weights / Window Detail / Reverse ─
    ...
    with t2:
        ...
        if all_daily.empty:
            ...
            fig2.add_hline(y=0, line_dash='dash', line_color='white', opacity=0.3)
            fig2.update_layout(template='plotly_dark', height=400,
                               yaxis_title='Rolling 60-Day Sharpe')
            st.plotly_chart(fig2, use_container_width=True,
                            key=f'{key_prefix}_t2_sharpe')
        ...
        if all_daily.empty:
            ...
            if weight_cols:
                ...
                fig.update_layout(barmode='stack', template='plotly_dark',
                                  height=400, yaxis=dict(range=[0, 1]))
                st.plotly_chart(fig, use_container_width=True,
                                key=f'{key_prefix}_t3_weights_bar')
                ...
                if available:
                    ...
                    fig3.update_layout(template='plotly_dark', height=400,
                                       yaxis=dict(range=[0, 1]))
                    st.plotly_chart(fig3, use_container_width=True,
                                    key=f'{key_prefix}_t3_weights_ts')
    ...
    with t5:
        ...
        if reverse_summary_df.empty:
            ...
            if signal_rev:
                st.info(
                    f"ℹ️ Today's reverse signal trained on: "
                    f"**{signal_rev.get('trained_on', '—')}** "
                    f"({signal_rev.get('train_start','?')} → "
            ...
            with rev_t1:
                ...
                disp = reverse_summary_df[
                    [c for c in show_cols if c in reverse_summary_df.columns]].copy()
                disp = disp.rename(columns={
                    'window_id': 'Window', 'label': 'Label',
                    'ftrl_total_return': 'FTRL Return', 'agg_total_return': 'AGG Return',
                ...
                if 'excess_return' in reverse_summary_df.columns:
                    ...
                    fig.add_hline(y=0, line_dash='dash', line_color='white', opacity=0.4)
                    fig.update_layout(template='plotly_dark', height=400,
                                      title=f'{group_name} — Excess Return by Training Start Year',
                                      xaxis_title='Reverse Window',
                                      yaxis_title='Excess Return (%)')
                    st.plotly_chart(fig, use_container_width=True,
                                    key=f'{key_prefix}_rev_t1_bar')
            ...
            with rev_t2:
                if all_reverse_daily.empty:
                    ...
                    fig.update_layout(template='plotly_dark', height=500,
                                      ...
                                      legend=dict(x=1.01, y=1))
                    st.plotly_chart(fig, use_container_width=True,
                                    key=f'{key_prefix}_rev_t2_equity')
                    ...
                    if weight_cols:
                        ...
                        if available_rev:
                            sel_label = st.selectbox(
                                ...
                                key=f'{key_prefix}_rev_weight_sel')
                            ...
                            for col in weight_cols:
                                fig3.add_scatter(
                                    x=sel_df.index, y=sel_df[col], mode='lines',
                                    name=col.replace('w_', ''), stackgroup='one',
                                    line=dict(color=colors.get(col.replace('w_',''),'#888')))
                            fig3.update_layout(template='plotly_dark', height=400,
                                               yaxis=dict(range=[0, 1]))
                            st.plotly_chart(fig3, use_container_width=True,
                                            key=f'{key_prefix}_rev_t3_ts')
                ...
                if all_reverse_daily.empty:
                    ...
                    if available_rev2:
                        sel_label2 = st.selectbox(
                            ...
                            key=f'{key_prefix}_rev_detail_sel')
                        ...
                        wd2      = load_reverse_window_daily(sel_wid2, suffix)
                        c1, c2, c3, c4 = st.columns(4)
                        ...
                        if not wd2.empty:
                            ...
                            st.plotly_chart(fig4, use_container_width=True,
                                            key=f'{key_prefix}_rev_t4_equity')
                            peak2 = curve2.cummax()
                            dd2   = (curve2 - peak2) / peak2
                            fig5  = go.Figure()
                            fig5.add_scatter(x=wd2.index, y=dd2 * 100, mode='lines',
                                             fill='tozeroy',
                                             line=dict(color='#FF1744', width=1))
                            fig5.update_layout(template='plotly_dark', height=250,
                                               title='Drawdown (%)')
                            st.plotly_chart(fig5, use_container_width=True,
                                            key=f'{key_prefix}_rev_t4_dd')
...
with st.sidebar:
    ...
    st.markdown("**Walk-forward:** 14 windows × 2 groups")
    st.divider()

    st.subheader("🎯 Generate Signals")
    st.caption("Run after deepwave-dl has updated so yesterday's signal scores correctly.")

    col_fi, col_eq = st.columns(2)
    with col_fi:
        st.markdown("**FI**")
...
# ── Top-level FI / EQUITY tabs ────────────────────────────────────────────────

fi_tab, eq_tab = st.tabs([
    "💵 Fixed Income (FI)",
    "📊 Equity",
])

with fi_tab:
    st.markdown("### Fixed Income — TLT · LQD · HYG · VNQ · GLD · SLV")
    render_group(
...
with eq_tab:
    st.markdown("### Equity — QQQ · XLK · XLF · XLE · XLV · XLI · XLY · XLP · XLU · XME · GDX · IWM")
    render_group(
        group_name="Equity",
        suffix="_equity",
